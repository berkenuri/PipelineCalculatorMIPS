...
Opcode OpcodeTable::getOpcode(string str1, string str2) {
// Given a valid function and opcode field, returns an Opcode as a template for its
// instruction
    for (int i = 0; i < (int)UNDEFINED; i++) {
       
       if (str1.compare("000000") == 0) {
        
            if ( (myArray[i].op_field == str1) && (myArray[i].funct_field == str2) ) 
                return (Opcode)i;
        }

       else if (myArray[i].op_field == str1)
            return (Opcode)i;
...
// Given an Opcode, returns the name of the register
    if(o < 0 || o >= UNDEFINED)
        return "UNDEFINED";
...
// Given an Opcode, returns true if instruction is a memory type
bool OpcodeTable::isMemoryType(Opcode o) {
 
    if(o < 0 || o > UNDEFINED)
        return false;
  
    return myArray[o].memoryType;
}
