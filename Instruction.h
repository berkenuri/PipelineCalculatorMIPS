...
class Instruction{
 public:
  ...
  // Checks if the instruction in the opcode table has an immediate label
  bool getIMMLabel();

  // Checks if the instruction in the opcode table is memory type
  bool getMemoryType();
  ...
  // Stores the decoded string MIPS instruction
  void setAssembly(string s) { myAssembly = s; };

  // Returns a string representing the decoded MIPS instructions
